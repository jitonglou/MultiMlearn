...
Also, the proposed model can incorporate the identified latent subgroup information ([*Lou et al, 2021](https://onlinelibrary.wiley.com/doi/10.1002/sim.8880)) to alleviate confounding effects among subjects and improve the performance of the M-learning.
The coding example for subgroup identification can be found at [*this GitHub repository](https://github.com/jitonglou/EHR_ITR), which is not a function of the `MultiMlearn` package.

## Documentation
A simulation study illustrating the necessity of latent subgroup information can be found in [*this supplementary material](https://github.com/jitonglou/MultiMlearn/vignettes/supp_v3.pdf).
...
### Simulate a dataset
Following the settings (equation (1), (2), (5), and (6)) in Section S.1.1 and S.1.2 of the [*supplementary material](https://github.com/jitonglou/MultiMlearn/vignettes/supp_v3.pdf), we simulate a dataset for N=200 subjects. The covariate vector $\mathbf{x}_i$ was set to be p=20 dimensional. Moreover, we set the number of treatments K=4, the number of groups J=4.
```{r simdata}
rseed = 0
set.seed(rseed)
# ?simulate_data # explanation of arguments
...
We split the data to 3 folds. In each test fold, the tuning parameter in SVMs (cost of constraints violation) is selected from $\{2^k:k=0,\pm1,\ldots,\pm8\}$ using another layer of 3-fold cross-validation.  
```{r mlearning}
...
To obtain the same results in Table S1 and Figure S1 in Section S.1.3 of the supplementary material, you need to run the above scripts for group 2 to 4, and repeat the procedure for 100 times using `rseed=1,...,100`, `ntree=seq(500,5000,500)`, `ksvm.grid = expand.grid(C = 2^(-15:15))`, `nfolds_outer = 5`, and `nfolds_outer = 5`. Each replication can be done parrellely. 
